...
describe('resolveFlagValue', () => {
  ...
  it('passes flag name, remote value, and context to override fn', () => {
    const fn = vi.fn().mockReturnValue(undefined);
    const ctx = { userId: 'u1' };
    resolveFlagValue('feature-x', true, undefined, fn, ctx);
    expect(fn).toHaveBeenCalledWith('feature-x', true, ctx);
  });
});
